...
export default function StoryBand() {
  return (
    <section
      ...
      className="on-dark plate relative overflow-hidden bg-cocoa text-cream"
    >
      <Image
        ...
        <div className="grid items-center gap-16 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="max-w-2xl">
            <Reveal>
              <h2
                id="story-heading"
                className="display-soft font-display text-4xl font-semibold leading-[1.02] sm:text-6xl"
              >
            ...
            <Reveal delay={0.18}>
              <Link
                href="/about"
                className="press group mt-10 inline-flex items-center gap-3 rounded-full border-2 border-brand-gold/70 px-7 py-3.5 text-sm font-semibold text-brand-gold transition-[transform,background-color,color] hover:bg-brand-gold hover:text-cocoa"
              >
                Read our story
          ...
          <Reveal delay={0.12} className="relative hidden lg:block">
            <figure className="relative mx-auto w-[86%] overflow-hidden rounded-t-[13rem] rounded-b-3xl border border-brand-gold/30 bg-[linear-gradient(170deg,#57331f,#241209)] px-9 pb-9 pt-16">
              <span
                aria-hidden="true"
                className="text-outline-cream pointer-events-none absolute left-1/2 top-7 -translate-x-1/2 select-none font-display text-[7rem] font-semibold leading-none opacity-30"
              >
                ’88
              </span>
              <Parallax distance={22}>
                <Image
                  ...
                  alt="Kimico Luxury Selection truffle pouches in six flavours"
                  width={1200}
                  height={640}
                  sizes="480px"
                  className="relative h-auto w-full drop-shadow-[0_30px_34px_rgba(0,0,0,0.55)]"
                />
              </Parallax>
              <figcaption className="mt-7 text-center font-display text-lg italic text-brand-gold/90">
                “{COMPANY.tagline}”
              </figcaption>
            </figure>
          </Reveal>
